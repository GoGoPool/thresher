...
func NewTerminalApp(blockchain string, roomname string, nick string, cmdchan chan UICommand, msgchan chan string) *TermApp {
	...
	titleleftbox := tview.NewTextView().
		SetText(fmt.Sprintf("Thresher Wallet  (v%s)", version.Version)).
...
func (ui *UI) generateKeyForm() {
	...
	form := tview.NewForm()
	form.SetBorder(true)
	form.SetTitle("Generate MultiSig Wallet")
	...
	form.AddInputField("Threshold", "", inputWidth, nil, nil)
	...
	form.AddButton("Generate", func() {
		...
		thresholdstr := form.GetFormItemByLabel("Threshold").(*tview.InputField).GetText()
		var threshold int
...
func (ui *UI) startEventHandler() {
	...
	fetchticker := time.NewTicker(time.Second*10)
